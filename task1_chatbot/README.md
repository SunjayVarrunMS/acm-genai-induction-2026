...
**How memory works:** every message in the current performance is kept in `st.session_state` and replayed to the model on each turn, so the bot can reference something said earlier in the same conversation — no re-running the file between messages.
